...
if (!isTouch) {
  ...
  // mobile: venom wanders and sticks on fast scroll
  let targetX = window.innerWidth * 0.5;
  let targetY = window.innerHeight * 0.4;
  let stuckEdge = null;
  let releaseTimer = null;
  const STICK_SPEED = 1.3;
  ...
  window.addEventListener('scroll', () => {
      ...
      const speed = (y - lastScrollY) / dt;
      lastScrollY = y;
      lastScrollT = now;

      if (speed > STICK_SPEED) {
          stuckEdge = 'top';
          venomCursor.classList.remove('venom-stuck-bottom');
          venomCursor.classList.add('venom-stuck-top');
      } else if (speed < -STICK_SPEED) {
          stuckEdge = 'bottom';
          venomCursor.classList.remove('venom-stuck-top');
          venomCursor.classList.add('venom-stuck-bottom');
      }
      ...
      if (stuckEdge) {
          ...
          releaseTimer = setTimeout(() => {
              stuckEdge = null;
              venomCursor.classList.remove('venom-stuck-top', 'venom-stuck-bottom');
              pickNewWanderTarget();
          }, 260);
      }
  }, { passive: true });
  ...
  function animateMobileVenom() {
      ...
      let tx = targetX + driftX;
      let ty = targetY + driftY;
      let ease = 0.02;

      if (stuckEdge === 'top') {
          tx = window.innerWidth * 0.5 + driftX * 1.4;
          ty = window.innerHeight * 0.08;
          ease = 0.1;
      } else if (stuckEdge === 'bottom') {
          tx = window.innerWidth * 0.5 + driftX * 1.4;
          ty = window.innerHeight * 0.92;
          ease = 0.1;
      }

      cursorX += (tx - cursorX) * ease;
      cursorY += (ty - cursorY) * ease;
      venomCursor.style.left = cursorX + 'px';
      venomCursor.style.top = cursorY + 'px';
...
function updateUI() {
  if (currentPhase === 'NS') {
      ...
      trafficStatus.innerHTML =
          'Greedy algorithm: <strong>North-South Road</strong> green at 90% combined congestion. East-West Road on red. Collision prevented.';
  } else {
      ...
      trafficStatus.innerHTML =
          'Greedy algorithm: <strong>East-West Road</strong> green at 77% combined congestion. North-South Road on red. Collision prevented.';
  }
}
...
if (canvas) {
  ...
  canvas.addEventListener('mousedown', (e) => { isDrawing = true; const p = getPos(e); ctx.beginPath();
      ctx.moveTo(p.x, p.y); });
  canvas.addEventListener('mousemove', (e) => { if (!isDrawing) return; const p = getPos(e); ctx.lineTo(p.x, p.y);
      ctx.stroke(); });
  canvas.addEventListener('mouseup', () => { isDrawing = false; });
  canvas.addEventListener('mouseleave', () => { isDrawing = false; });

  canvas.addEventListener('touchstart', (e) => { e.preventDefault(); isDrawing = true; const p = getPos(e);
      ctx.beginPath(); ctx.moveTo(p.x, p.y); }, { passive: false });
  canvas.addEventListener('touchmove', (e) => { e.preventDefault(); if (!isDrawing) return; const p = getPos(e);
      ctx.lineTo(p.x, p.y); ctx.stroke(); }, { passive: false });
...
// resume download — venom spits pdf
...
if (resumeBtn) {
  resumeBtn.addEventListener('click', (e) => {
      ...
      venomCursor.classList.add('spitting');
      setTimeout(() => venomCursor.classList.remove('spitting'), 380);

      const pdf = document.createElement('div');
      pdf.className = 'venom-pdf';
      pdf.style.left = originX + 'px';
      pdf.style.top = originY + 'px';
      pdf.style.setProperty('--pdf-dx', (destX - originX) + 'px');
      pdf.style.setProperty('--pdf-dy', (destY - originY) + 'px');
      document.body.appendChild(pdf);
      requestAnimationFrame(() => pdf.classList.add('launch'));
