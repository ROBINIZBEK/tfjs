...
if('scrollRestoration' in history){
  ...
  const MIN_PHASE_DURATION = 5000;
  const MAX_PHASE_DURATION = 11000;
  ...
  function updateLane(lane) {
    const cfg = chowkLanes[lane];
    const queue = queues[lane];
    const isGreen = cfg.road === currentPhase;

    queue.forEach((car, i) => {
      const ahead = queue[i - 1];
      const signalOpen = isGreen || car.released;

      if (cfg.dir === 1) {
        const aheadCap = ahead ? ahead.pos - CAR_GAP : Infinity;
        const sigCap = signalOpen ? Infinity : cfg.stopLine;
        car.pos = Math.min(car.pos + CAR_SPEED, Math.min(aheadCap, sigCap));
        if (car.pos >= cfg.stopLine) car.released = true;
      } else {
        const aheadCap = ahead ? ahead.pos + CAR_GAP : -Infinity;
        const sigCap = signalOpen ? -Infinity : cfg.stopLine;
        car.pos = Math.max(car.pos - CAR_SPEED, Math.max(aheadCap, sigCap));
        if (car.pos <= cfg.stopLine) car.released = true;
      }
